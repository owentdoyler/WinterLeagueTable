    ...
    function sortTeamScores($teamScores){
        usort($teamScores, function($a, $b){
            ...
            elseif ($a->teamScore() > $b->teamScore()) return -1;
                ...
                foreach($a->weeks as $week) {
                    if($week->weekNumber > $aWeek->weekNumber) {
                        $aWeek = $week;
                    }
                }

                foreach($b->weeks as $week){
                    if($week->weekNumber > $bWeek->weekNumber) {
                        $bWeek = $week;
                    }
                }

                // $messageA = "team name: " . $a->teamName . ", week number: " . $aWeek->weekNumber . ", score: " . $aWeek->weekScore();
                // $messageB = "team name: " . $b->teamName . ", week number: " . $bWeek->weekNumber . ", score: " . $bWeek->weekScore();
                // echo $messageA;
                // echo "\n";
                // echo $messageB;

                if ($aWeek->weekScore() < $bWeek->weekScore()) return 1;    
                else if ($aWeek->weekScore() > $bWeek->weekScore()) return -1;      
                return 0;
            }
        });
        return $teamScores;
